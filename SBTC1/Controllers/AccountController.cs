...
using SBTC1.ViewModels.Account;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    else if (user.UserType == AppConstant.TRAIN_OPERATOR)
                    {
                        return RedirectToAction("ViewBookingList", AppConstant.TRAINOPERATOR_CONTROLLER);
                    }
        ...
        public async Task<IActionResult> ExternalLoginCallback(string returnUrl = null, string remoteError = null)
        {
            ...
            if (user != null)
            {
                ...
                else if (userType == AppConstant.TRAIN_OPERATOR)
                {
                    returnUrl = Url.Content("~/TrainOperator/ViewBookingList");
                }
                else if (userType == AppConstant.ADMIN)
                {
                    returnUrl = Url.Content("~/Admin/ViewTrainList");
                }
            }
